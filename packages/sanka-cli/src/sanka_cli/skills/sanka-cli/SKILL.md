...
Use task-selected types and connection references in `sanka.yaml`. Run `sanka app plan`, then `sanka app validate --json` without destination writes, apply the reviewed hash with `sanka app apply`, and run `sanka app verify`. Hosted App runs use `--program ID` or `--migration ID`. Inspect failures before retrying writes.
...
The shared CLI serves Sanka App data migrations, Sanka Flow workflow operations,
