...
from sanka_data import OwnerProfile
        ...
        *,
        ...
        if len(matches) > 1:
            raise MappingError(
                ...
                code="SANKA_MIGRATE_OWNER_EMAIL_DUPLICATE",
            )
        if matches and not active:
            raise MappingError(
                f"Destination owner is inactive: {normalized}",
                code="SANKA_MIGRATE_OWNER_INACTIVE",
            )
        ...
        if policy == "fallback":
            ...
            if not fallback:
                raise MappingError(
                    ...
                    code="SANKA_MIGRATE_FALLBACK_OWNER_REQUIRED",
                )
            if fallback == normalized:
                raise MappingError(
                    f"Destination owner email was not found: {normalized}",
                    code="SANKA_MIGRATE_OWNER_NOT_FOUND",
                )
