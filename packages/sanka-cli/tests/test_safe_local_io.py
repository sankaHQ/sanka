...
def test_safe_copy_regular_file_streams_content_larger_than_one_chunk(
    tmp_path: Path,
) -> None:
    ...
    destination = safe_copy_regular_file(source, tmp_path / "copy.sqlite3")

    assert destination.read_bytes() == content


def test_safe_copy_regular_file_enforces_read_limit(tmp_path: Path) -> None:
    source = tmp_path / "source.sqlite3"
    source.write_bytes(b"too large")
    destination = tmp_path / "copy.sqlite3"

    with pytest.raises(UnsafeLocalPathError, match="read limit"):
        safe_copy_regular_file(source, destination, max_bytes=3)

    assert not destination.exists()


def test_safe_copy_regular_file_rejects_hard_link_source(tmp_path: Path) -> None:
    source = tmp_path / "source.sqlite3"
    source.write_bytes(b"sqlite-bytes")
    link = tmp_path / "linked.sqlite3"
    link.hardlink_to(source)

    with pytest.raises(UnsafeLocalPathError, match="multiple hard links"):
