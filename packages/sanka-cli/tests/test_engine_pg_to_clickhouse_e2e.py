...
POSTGRES_DSN = os.environ.get("SANKA_MIGRATE_TEST_POSTGRES_DSN")
CLICKHOUSE_URL = os.environ.get("SANKA_MIGRATE_TEST_CLICKHOUSE_URL")

pytestmark = pytest.mark.skipif(
    not (POSTGRES_DSN and CLICKHOUSE_URL),
    reason="set SANKA_MIGRATE_TEST_POSTGRES_DSN and SANKA_MIGRATE_TEST_CLICKHOUSE_URL to run",
)
...
async def test_flagship_postgres_to_clickhouse(tmp_path: Path) -> None:
    ...
    schema = f"sanka_migrate_e2e_{suffix}"
