...
from sanka.runtime.mapping import MappingGroup, MigrationMappingField
...
def test_legacy_key_is_rejected_when_the_source_has_filtered_routes() -> None:
    ...
    assert excinfo.value.code == "SANKA_MIGRATE_ROUTE_CHECKPOINT_MISMATCH"
...
@pytest.mark.parametrize(
    ("raw", "code"),
    [
        (None, "SANKA_MIGRATE_ROUTE_MANIFEST_MISSING"),
        ([], "SANKA_MIGRATE_ROUTE_MANIFEST_MISSING"),
        (["route"], "SANKA_MIGRATE_ROUTE_MANIFEST_INVALID"),
        (
            [{"sourceObject": "", "destinationObject": "companies"}],
        ...
        (_manifest() + _manifest(), "SANKA_MIGRATE_ROUTE_MANIFEST_INVALID"),
    ],
    ids=[
        "missing",
        "empty",
...
def test_high_water_mark_faults(raw: Any) -> None:
    ...
    assert excinfo.value.code == "SANKA_MIGRATE_EXECUTION_HIGH_WATER_MARK_INVALID"
