...
def test_connector_client_rejects_wrong_protocol(tmp_path: Path) -> None:
    ...
    client = DataExtensionHostClient(executable, environment=tmp_path)
...
def test_connector_client_rejects_corrupt_host_output(
    ...
    client = DataExtensionHostClient(fake_python(tmp_path, source), environment=tmp_path)
...
def test_remote_connector_rejects_non_string_capability_names(tmp_path: Path) -> None:
    client = DataExtensionHostClient(sys.executable, environment=tmp_path)
    description = {
        "roles": ["source"],
    ...
    with pytest.raises(ExtensionError) as raised:
        build_remote_data_extension(client, "example", "source", description=description)
...
async def test_sqlite_connector_round_trip_stays_out_of_process(tmp_path: Path) -> None:
    ...
    with DataExtensionHostClient(sys.executable, environment=site_packages) as client:
        description = client.request("sqlite", "describe", {})
        source = cast(
            SystemReader,
            build_remote_data_extension(client, "sqlite", "source", description=description),
        )
        destination = cast(
            SystemWriter,
            build_remote_data_extension(client, "sqlite", "destination", description=description),
        )
