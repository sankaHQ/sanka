...
async def test_engine_resolves_named_connections_without_persisting_secrets(tmp_path: Any) -> None:
    ...
    engine = MigrationEngine(
        ...
        registry=ExtensionRegistry(
            {
                "source": ExtensionRegistration(name="source", source=source),
                "destination": ExtensionRegistration(
                    name="destination",
                    destination=destination,
                ),
            }
...
async def test_engine_rejects_named_connection_provider_mismatch(tmp_path: Any) -> None:
    ...
    engine = MigrationEngine(
        store=SqliteStateStore(tmp_path / "state.db"),
        registry=ExtensionRegistry({"source": ExtensionRegistration(name="source", source=source)}),
        credential_provider=provider,
    )
