...
A sibling of :class:`ferry.runtime.state.StateStore`, not an extension. The
...
identity ledgers (:mod:`ferry.runtime.mapping.pending_relationships`)
...
from ferry.runtime.mapping.pending_relationships import IdentityLedger, SharedIdentityLedger
...
class ExecutionLedger(Protocol):
    ...
    async def upsert_results(self, results: Sequence[RecordWriteOutcome]) -> int:
        ...
        The engine fails closed (``FERRY_RECORD_RESULT_BULK_SAVE_INCOMPLETE``)
...
class ExecutionJournal(Protocol):
    ...
    :class:`~ferry.runtime.execution.errors.ExecutionFault` with code
    ``FERRY_EXECUTION_JOB_SUPERSEDED`` when the journal's job is no longer
