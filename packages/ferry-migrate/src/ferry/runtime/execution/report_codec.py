...
_INVALID_MANIFEST_MESSAGE = "The saved Ferry route manifest is invalid."
...
def canonical_route_manifest(raw: Any) -> MappingRouteManifest:
    ...
    for item in raw:
        if not isinstance(item, dict):
            raise ExecutionFault(_INVALID_MANIFEST_MESSAGE, code="FERRY_ROUTE_MANIFEST_INVALID")
        ...
        try:
            ...
            raise ExecutionFault(
                "The saved Ferry route manifest contains an invalid source filter.",
                code="FERRY_ROUTE_MANIFEST_INVALID",
            ) from exc
        ...
        if saved_route_key != route_key or route_key in seen:
            raise ExecutionFault(
                "The saved Ferry route identity does not match its source filter.",
                code="FERRY_ROUTE_MANIFEST_INVALID",
            )
        seen.add(route_key)
        normalized.append(
...
def require_matching_route_manifest(
    ...
    if current != expected:
        raise ExecutionFault(
            "The Ferry mapping routes changed after continuous execution was queued. "
...
def selected_route_keys(
    ...
    if not isinstance(requested_route_keys, list | tuple):
        raise ExecutionFault(
            "Ferry execution route keys must be a list.",
    ...
    if unknown:
        raise ExecutionFault(
            "Ferry execution includes routes outside the reviewed mapping.",
            code="FERRY_EXECUTION_ROUTE_INVALID",
...
def execution_route_high_water_marks(
    ...
    if not isinstance(raw, dict):
        raise ExecutionFault(
            "The saved Ferry route high-water marks are invalid.",
    ...
    for route_key in sorted(known_route_keys):
        ...
        if not normalized_value:
            raise ExecutionFault(
                "The saved Ferry route high-water mark is blank.",
                code="FERRY_EXECUTION_HIGH_WATER_MARK_INVALID",
            )
...
def prepare_route_state(
    ...
    key raises ``FERRY_ROUTE_CHECKPOINT_MISMATCH``. A saved manifest that no
    longer matches raises ``FERRY_ROUTE_MANIFEST_CHANGED``.
    """
    ...
    if (
        ...
        raise ExecutionFault(
            "The saved Ferry checkpoints belong to different mapping routes.",
            code="FERRY_ROUTE_MANIFEST_CHANGED",
        )
    ...
    def resolve_route_key(raw_key: Any) -> str:
        ...
        raise ExecutionFault(
            "A saved Ferry checkpoint does not match the reviewed source-filter route.",
            code="FERRY_ROUTE_CHECKPOINT_MISMATCH",
        )
    ...
    for field_name in (
        ...
        for raw_key, field_value in value.items():
            ...
            if route_key in remapped and str(raw_key) != route_key:
                raise ExecutionFault(
                    "Multiple saved Ferry checkpoints resolve to the same route.",
                    code="FERRY_ROUTE_CHECKPOINT_MISMATCH",
                )
