...
:class:`ferry.runtime.state.SqliteStateStore` uses. The v0 ``checkpoints``
...
- ``execution_results`` is **pair-keyed** —
  ...
  whose status is in :data:`~ferry.runtime.state.TERMINAL_WRITE_STATUSES`
  ...
  ``FERRY_RECORD_RESULT_BULK_SAVE_INCOMPLETE`` (the engine re-checks the
...
from ferry.runtime.execution.report_codec import dump_journal, load_journal
from ferry.runtime.execution.state import ClaimOutcome, SaveOutcome
...
class SqliteExecutionState:
    ...
    :class:`ferry.runtime.state.SqliteStateStore` — the tables are disjoint.
    """
    ...
    async def load(self) -> JournalEntry | None:
        def query() -> JournalEntry | None:
            ...
            if self._job_id is not None and row["job_id"] not in (None, self._job_id):
                raise ExecutionFault(
                    "Ferry execution job was superseded.",
                    code="FERRY_EXECUTION_JOB_SUPERSEDED",
                )
