...
- ledger-free reference-mapping violations (required reference empty,
  ...
  ``FERRY_EMPTY_DESTINATION_RECORD``, exactly as the production dry run
  does.
...
:class:`~ferry.runtime.mapping.errors.MappingError`, and
:class:`~ferry.runtime.execution.errors.ExecutionFault` all shape
identically; anything else falls back to the production
``FERRY_MAPPING_VALUE_INVALID`` rejection.

The private imports from :mod:`ferry.runtime.execution.routes` are
...
from ferry.connector import Credentials, SourceConnector
from ferry.runtime.execution.errors import ExecutionFault
from ferry.runtime.execution.model import ExecutionRoute
from ferry.runtime.execution.report_codec import _source_filter_payload
...
from ferry.runtime.mapping.record_mapping import destination_properties, relationship_source_ids
...
MISSING_IDENTITY_CODE = "FERRY_SOURCE_RECORD_IDENTITY_MISSING"
...
of :data:`~ferry.runtime.execution.errors.EXECUTION_FAULT_CODES`.
"""

_LIMIT_INVALID_MESSAGE = "Dry-run maxRecords must be a positive integer."
_FALLBACK_REJECTION_CODE = "FERRY_MAPPING_VALUE_INVALID"
...
def validation_rejection(
    *,
    ...
    the production ``FERRY_MAPPING_VALUE_INVALID`` fallback.
    """
...
async def validate_routes(
    *,
    ...
    if sample_size < 1:
        raise ExecutionFault(_LIMIT_INVALID_MESSAGE, code="FERRY_DRY_RUN_LIMIT_INVALID")
...
def _validate_record(
    ...
    *,
    ...
    if not properties:
        return rejection(
            ExecutionFault(_EMPTY_DESTINATION_MESSAGE, code="FERRY_EMPTY_DESTINATION_RECORD")
        )
    return None
