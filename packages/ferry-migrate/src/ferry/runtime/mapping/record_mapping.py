...
def _apply_value_map(
    ...
    *,
    ...
    if len(matches) > 1:
        raise MappingError(
            "More than one Ferry value-map predicate matched the source record.",
            code="FERRY_VALUE_MAP_AMBIGUOUS",
        )
...
def apply_transform(value: Any, transform_rule: str | None) -> Any:
    ...
    raise MappingError(
        f"Unsupported Ferry transform rule: {rule}",
        code="FERRY_TRANSFORM_UNSUPPORTED",
    )
