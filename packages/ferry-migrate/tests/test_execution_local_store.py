...
def _store(
    ...
    return SqliteExecutionState(tmp_path / "ferry.db", run_id=run_id, job_id=job_id)
...
async def test_short_persisted_count_fails_closed(
    ...
    assert excinfo.value.code == "FERRY_RECORD_RESULT_BULK_SAVE_INCOMPLETE"
...
async def test_journal_is_fenced_against_another_job(tmp_path: Path) -> None:
    ...
    with pytest.raises(ExecutionFault) as excinfo:
        await other.load()
    assert excinfo.value.code == "FERRY_EXECUTION_JOB_SUPERSEDED"
