...
Gated on ``FERRY_TEST_POSTGRES_DSN`` like the connector integration tests.
...
import psycopg
import pytest
from ferry_connector_postgres import CONNECTOR, PostgresDestination
...
async def schema() -> AsyncIterator[str]:
    ...
    name = f"ferry_e2e_{secrets.token_hex(4)}"
