...
from ferry.runtime.mapping import MigrationMappingField, mapping_groups, mapping_route_manifest
...
async def test_totals_refuse_a_frozen_mark_without_bounded_counting() -> None:
    ...
    assert exc.value.code == "FERRY_SOURCE_HIGH_WATER_MARK_UNSUPPORTED"
...
async def test_freeze_scope_rejects_saved_marks_that_do_not_match_the_routes() -> None:
    ...
    assert exc.value.code == "FERRY_EXECUTION_HIGH_WATER_MARK_INVALID"
...
async def test_freeze_scope_rejects_a_changed_manifest() -> None:
    ...
    assert exc.value.code == "FERRY_ROUTE_MANIFEST_CHANGED"
...
async def test_freeze_scope_rejects_a_selection_outside_the_manifest() -> None:
    ...
    assert exc.value.code == "FERRY_EXECUTION_ROUTE_INVALID"
