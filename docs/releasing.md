...
The current candidate is `sanka-cli==0.2.3`, tagged `v0.2.3`. Its source
...
make check
make build-release
uv run python scripts/check_release_tag.py v0.2.3 tag
```
...
3. Create and push `v0.2.3` only with explicit authorization. Do not move or
   reuse the existing `v0.2.2` tag.
4. Verify local `release/SOURCE_COMMIT` and `release/SHA256SUMS` against the
   approved tag.
5. Dispatch **Publish sanka-cli** at `v0.2.3` with confirmation
   `publish-v0.2.3`.
...
7. The `pypi` job receives only `id-token: write`, downloads the named
   ...
   once.
8. Install `sanka-cli==0.2.3` and `sanka-cli[mcp]` in fresh environments;
