...
The consolidated candidate is `sanka-cli==0.2.0`, tagged `v0.2.0`. Its source
...
make check
make build-release
uv run python scripts/check_release_tag.py v0.2.0 tag
```
...
Keep the old publisher active until `0.2.0` is published and verified. The
...
3. Create and push `v0.2.0` only with explicit authorization.
4. Verify local `release/SOURCE_COMMIT` and `release/SHA256SUMS` against the
   approved tag.
5. Dispatch **Publish sanka-cli** at `v0.2.0` with confirmation
   `publish-v0.2.0`.
...
7. The `pypi` job receives only `id-token: write`, downloads the named
   ...
   once.
8. Install `sanka-cli==0.2.0` and `sanka-cli[mcp]` in fresh environments;
