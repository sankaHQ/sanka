...
def test_install_does_not_use_system_python(
    ...
    assert (bindir / "sanka").resolve() == root / "versions/0.2.16/bin/sanka"
    assert "sanka, version 0.2.16" in result.stdout
...
def test_upgrade_and_repeat_install_switch_only_the_owned_link(
    ...
    for _ in range(2):
        result = invoke(environment)
        assert result.returncode == 0, result.stderr + result.stdout
        assert (root / "current").resolve() == root / "versions/0.2.16"
...
@pytest.mark.parametrize("version", ["../x", "1;echo bad", "", "-e", "0.2.16/evil"])
