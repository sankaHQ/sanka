...
def test_unified_release_artifact_contains_three_license_zones(tmp_path: Path, monkeypatch) -> None:
    dist = tmp_path / "dist"
    dist.mkdir()
    wheel = dist / "sanka_cli-0.2.2-py3-none-any.whl"
    wheel_members = {
        "sanka/__init__.py": b"",
        "sanka/_client.py": b"",
        ...
        "sanka_cli-0.2.2.dist-info/METADATA": (
        ...
        "sanka_cli-0.2.2.dist-info/entry_points.txt": (
            b"[console_scripts]\nsanka = sanka_cli.main:main\n"
        ),
        "sanka_cli-0.2.2.dist-info/licenses/LICENSES/Apache-2.0.txt": b"Apache\n",
        "sanka_cli-0.2.2.dist-info/licenses/LICENSES/AGPL-3.0-only.txt": b"AGPL\n",
        "sanka_cli-0.2.2.dist-info/licenses/NOTICE": b"MIT License\n",
    }
    with zipfile.ZipFile(wheel, "w") as archive:
        for name, data in wheel_members.items():
            archive.writestr(name, data)

    sdist = dist / "sanka_cli-0.2.2.tar.gz"
    with tarfile.open(sdist, "w:gz") as archive:
        for name in ("LICENSES/Apache-2.0.txt", "LICENSES/AGPL-3.0-only.txt", "NOTICE"):
            data = b"license\n"
            info = tarfile.TarInfo(f"sanka_cli-0.2.2/{name}")
            info.size = len(data)
